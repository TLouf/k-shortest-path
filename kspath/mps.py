...
class PathBuffer(object):
    ...
    def push(self, cost, path, deviation_index, deviation_path_cost):
        ...
        if hashable_path not in self._paths:
            heappush(
                self._sorted_paths,
                (cost, next(self._counter), path, deviation_index, deviation_path_cost),
            )
...
class SingleTargetDeviationPathAlgorithm(object):
    ...
    def mps_deviation_paths(
        self, path_cost, path, deviation_index, deviation_path_cost, list_x
    ):
        ...
        for i in range(deviation_index, len(path) - 1):
            ...
            cost_head_node_list = self._sorted_arcs[v_i]["cost_head_node_list"][
                vj_index + 1 :
            ]
            for cost, head_node in cost_head_node_list:
                if head_node not in root_path_nodes:
                    ...
                    if i == deviation_index:
                        list_x.push(
                            deviation_path_cost + cost, new_path, i, deviation_path_cost
                        )
                    else:
